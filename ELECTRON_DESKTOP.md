...
Packaged Electron clients check GitHub Releases first for installer-level updates through `electron-updater`. If the GitHub check or download fails, the client automatically switches to the ModelScope fallback source and retries the same installer-level update flow. Both sources use the release assets generated by electron-builder:
...
The primary publish target is `xiaoqi0102/Infinite-Canvas` on GitHub Releases. The fallback ModelScope target defaults to the same owner/repo path and reads files from the `desktop-release/` directory through the ModelScope file API. After a release is uploaded, the packaged client can detect it, prompt the user to download it, and then prompt again to restart and install. Before installing, Electron stops the local backend process and then calls `autoUpdater.quitAndInstall()`.

The ModelScope fallback can be redirected without code changes:
...
Client update diagnostics are appended to `InfiniteCanvas_Data\desktop.log` with `client-update-*` events such as `client-update-source-selected`, `client-update-checking-for-update`, `client-update-source-fallback`, `client-update-update-available`, `client-update-download-progress`, `client-update-update-downloaded`, and `client-update-update-error`. The log includes the active source (`github` or `modelscope`) whenever source-specific handling matters.
...
Then upload the same three files to the ModelScope repo under `desktop-release/`:

```text
desktop-release/latest.yml
desktop-release/Infinite-Canvas-Setup-<VERSION>.exe
desktop-release/Infinite-Canvas-Setup-<VERSION>.exe.blockmap
```

Keep the filenames identical to the paths referenced by `latest.yml`; otherwise the fallback source can find the update metadata but cannot download the installer.

If the installed `modelscope upload` command only accepts `model` and `dataset` repos, upload these files to the Studio repo through the Python Hub API instead:
...
for local, remote in files:
    api.upload_file(repo_id=repo, repo_type='studio', path_or_fileobj=str(local), path_in_repo=remote, revision='master')
'@ | python -
```

Upload the installer and `.blockmap` first, then upload `latest.yml` last so old clients do not see metadata before the referenced files exist.
