...
> **2026-07-28 图片链路补充**：图片任务现与视频任务一样写入 `data/canvas_image_tasks.json`，后端重启会恢复已有上游任务 ID 的查询；普通画布与智能画布在图片任务提交后立即保存排队日志，并在后续轮询中更新同一条记录。没有上游任务 ID 的中断任务会安全失败，避免服务重启后重复提交和重复扣费。
>
> **2026-07-28 更新**：视频链路完成一轮全量容错修复(三层瞬时错误重试、任务快照限量节流、双画布行为对齐、Sudashui 官方域锁定等)。本文第 7/9/10/19 章描述的部分旧行为已变化,统一参数与行为差异见**第 24 章**。
...
- HTTP `5xx` 默认只表示本次查询暂时失败；但响应 JSON 如果已经明确给出 `failed`、
  `task_failed`、`generation_failed`、审核拒绝或内容过滤等业务终态，必须立即失败，不能继续重试到全局超时。
...
- 并行循环任一轮失败后，共享停止状态必须阻止 worker 领取新轮次；调用方等待已启动 worker
  安全收束后再结束本次运行。
- 页面加载和远程同步触发 pending 恢复时，所有异步恢复调用都必须显式处理拒绝，不能产生
  `unhandledrejection`。
...
8. 同一节点已有活动视频任务轮询时，手动再次生成会先弹窗确认；确认后创建独立任务并保留旧任务轮询，取消则不提交。普通画布把新任务追加到 `_pending`，智能画布为新任务创建独立输出节点；级联运行和失败待查询任务仍保持防重复保护。
...
- 自动预检上传成功后，会在用户数据目录 `data/canvas_video_upload_cache.json` 按本地文件路径、大小、修改时间和上传服务保存公网链接及失效时间。文件未变化、链接未接近失效且仍可访问时可以跨任务、跨重启复用；文件变化、链接不可达或剩余有效期不足以覆盖下一次任务提交安全余量时自动清除缓存并重新上传。用户手动点击“上传云端”仍强制生成新链接，不读取该缓存。
- Sudashui 文件上传响应目前只返回 `url`，没有独立有效期字段。后端优先从 URL 的 AWS/GCS 签名参数或绝对过期参数推导 `expires_at`；无法推导时按约 1 小时估算，可通过 `SUDASHUI_UPLOAD_URL_TTL_SECONDS` 调整。缓存复用默认保留 10 分钟提交安全余量，可通过 `VIDEO_MATERIAL_UPLOAD_CACHE_SAFETY_SECONDS` 调整；历史上按通用 24 小时写入的 Sudashui 缓存会按创建时间自动收紧到 1 小时。任务恢复只查询已存在的上游任务，不重新提交素材 URL。
...
- 公网探测返回明确 `Content-Type` 时必须与素材类型一致：图片接受 `image/*`，视频接受 `video/*` 及常见 HLS/DASH 容器类型，音频接受 `audio/*`；HTML、JSON、文本内容或其它明确不匹配的媒体类型会在创建任务前拒绝。为兼容签名 CDN，缺失类型和 `application/octet-stream` 作为无法判定的通用二进制继续放行。旧缓存类型错误且存在本地副本时会清除缓存并重新上传。
...
安全边界与已知风险：预检接口只接受同源页面调用；公网探测拒绝账号信息、本机/内网 DNS 结果和重定向，并把已校验 IP 固定到实际连接；自动重传只读取 `USER_DATA_ROOT` 约束下的 `/assets/`、`/output/`、`/api/storage-files/` 文件，不下载任意远程 URL。为避免重定向绕过 DNS 校验，当前合法的 30x 素材链接也会按不可用处理。预检只能确认提交瞬间可访问，无法消除链接在预检成功后、上游读取前再次过期的时间窗口。
...
MegabyAI 查询期间的连接超时、连接中断、`408`、`425`、`429` 和没有业务终态内容的 `5xx`
只代表本次状态查询暂时不可用，不能据此判定远端生成失败。后端会保持本地任务为 `polling`
并按 8 秒基础间隔或上游 `Retry-After` 自动重试，直到查询到 `completed` / `failed` 或达到
全局轮询超时；如果 `5xx` 响应正文已经包含 `failed`、`task_failed`、`generation_failed`、
`MODERATION_ERROR` 或 `CONTENT_FILTER` 等明确业务失败，则立即终止。`401`、`403` 等不可恢复
的请求错误也立即失败。旧版本若已把临时查询错误持久化为 `failed`，服务启动或用户点击
“查询结果”时会仅凭既有上游任务 ID 恢复查询，绝不会重新提交创建请求。
...
## 24. 2026-07-28 全链路容错修复

本章记录基于 23 条审查发现的全量修复,是当前生效的行为基线;与前文旧描述冲突时以本章为准。

### 24.1 统一容错参数

| 层 | 参数 | 当前值 |
| --- | --- | --- |
| 后端 `wait_for_video_task` / `wait_for_agnes_video_task` | 非终态错误连续失败上限 | 5 次(`VIDEO_POLL_TRANSIENT_MAX`),轮成功即清零;计费类终态错误仍立即抛 |
| 后端 | 失败退避 | 10/20/40/80/120 秒指数序列,取与基础间隔较大者,受总 deadline 约束 |
| 后端 | 轮询单次 GET 超时 | `VIDEO_POLL_REQUEST_TIMEOUT`(connect 20 / read 60 / write 30 / pool 20 秒),不再沿用 client 级 7200 秒 |
| 后端 | 任务快照保留 | 终态任务按 `updated_at` 只保留最近 200 条(`CANVAS_VIDEO_TERMINAL_KEEP`),进行中任务永不裁剪;快照与加载两侧同规则 |
| 后端 | 进度落盘节流 | 纯进度 patch(状态未跃迁)≥10 秒才落盘;状态跃迁/result/error 立即落盘;`raw_last` 超 20KB 截断;写盘经 `asyncio.to_thread` 移出事件循环并按代次去重 |
| 前端(双画布一致) | 轮询本地接口失败重试 | 连续 5 次计数,退避 5/10/20/30/30 秒;404 立即硬终态;级联中止不计数;耗尽走"可恢复失败"(保留任务与查询入口) |
| 前端(双画布一致) | 轮询软上限 | 7260 秒,超限转"可恢复保留",不删节点不报生成失败 |
| 前端 | 正常轮询间隔 | 后端返回的 `retry_after` 夹在 [5s, 30s],缺省 5 秒 |
| 智能画布 | 创建任务 POST 超时 | 600 秒(素材预检可能很慢),查询仍 30 秒;创建超时文案明确提示勿重复提交 |
| Sudashui 插件 | 轮询错误上限 | `_MAX_POLL_ERRORS = 5`(传输错误与 408/425/429/5xx 计数),排队阶段(任务未开始)独立 `poll_timeout` 预算,不再存在永不退出的轮询协程 |

### 24.2 行为变化要点

- **后端不再"一次网络抖动判死任务"**:`raise last_error` 前有连续失败计数,已扣费任务不会因瞬时 502/断连被永久标记 failed。
- **`rewrite_persisted_media_urls` 只替换 `type=="online-video"` 条目**,素材改名/移动不再清空进行中的图像/Comfy 任务;image/comfy 任务收尾一律 `CANVAS_TASKS.get()` 防御,不再抛 KeyError。
- **普通画布**:轮询瞬断时节点显示"网络中断,正在自动重试";succeeded 但解析不到输出 URL 时按终态失败处理(不再产生点不动的"可恢复失败"僵尸任务);轮询日志与节点刷新有 `lastLogSignature` 门控(与图像轮询一致);`waitCanvasVideoTaskResult` 死代码已删除(测试 `sourceBetween` 锚点同步改为 `completeCanvasVideoTask`);级联下 preflight 未完成会明确抛错,不再被误标为 done。
- **智能画布**:分支输出节点在存在可恢复(failed+recoverTaskId)视频任务时不会被删除,"任务未丢失"提示与实际行为一致;终态失败与空产出在节点上留下 `lastVideoTaskError` 失败占位;1440 次轮询上限改为 7260 秒软上限,超限走保留路径。
