...
def video_task_request_meta(payload: "CanvasVideoRequest"):
    return {
        ...
        "negative_prompt": str(payload.negative_prompt or "")[:500],
...
async def resume_canvas_video_task_result(task_id: str):
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        if is_sudashui_video_generations_mode(provider):
            try:
                return await resume_sudashui_video(
                    client,
                    upstream_task_id,
        ...
        if is_megabyai_video_mode(provider):
            try:
                return await resume_megabyai_video(
                    client,
                    upstream_task_id,
                    base_url=video_api_root(provider),
                    headers=api_headers(json_body=False, provider=provider),
        ...
        if is_geeknow_video_mode(provider):
            try:
                return await resume_geeknow_video(
                    client,
                    upstream_task_id,
                    base_url=video_api_root(provider),
                    headers=api_headers(json_body=False, provider=provider),
...
def jimeng_append_model_resolution_args(args, payload: CanvasVideoRequest, model_version="", include_model=None):
    resolved_model = model_version
    if include_model is True and not resolved_model:
        resolved_model = jimeng_video_model_version(payload.model)
    if include_model is not False and resolved_model:
        args.append(f"--model_version={resolved_model}")
    args.append(f"--video_resolution={jimeng_video_resolution_arg(resolved_model or payload.model, payload.resolution)}")
...
def persisted_json_references_media_path(target_path: str) -> bool:
    ...
    Canvas logs are only one possible owner. Nodes, other canvases, resumable
    video tasks, conversations, and asset metadata all count as live references
    and keep the file on disk.
    """
...
async def generate_ai_image(
    ...
    if is_tudou_async_image_mode(provider, model):
        return await generate_tudou_async_image(
            prompt, size, quality, model, reference_images, provider, aspect_ratio, resolution
        )
    if is_tudou_provider(provider) and is_tudou_grok_image_model(model):
        return await generate_tudou_grok_image(
            prompt, size, model, reference_images, provider, aspect_ratio
        )
...
def video_api_root(provider):
    base_url = canonical_video_api_root(provider.get("base_url") or AI_BASE_URL)
    if is_volcengine_provider(provider):
...
def is_sudashui_video_generations_mode(provider) -> bool:
    return effective_video_request_mode(provider) == SUDASHUI_VIDEO_REQUEST_MODE
...
async def generate_lingjing_openai_video(client, payload, provider, base_url, requested_model, progress=None):
    ...
    if is_lingjing:
        try:
            async with httpx.AsyncClient(timeout=VIDEO_POLL_TIMEOUT) as lingjing_client:
                return await generate_lingjing_openai_video(lingjing_client, payload, provider, base_url, requested_model, progress)
        except httpx.HTTPStatusError as exc:
            text = exc.response.text
    ...
    try:
        async with httpx.AsyncClient(timeout=VIDEO_POLL_TIMEOUT) as client:
            if is_aicost:
                ...
                try:
                    return await generate_aicost_video(
                        client,
                        request_data,
                        base_url=base_url,
                        headers=api_headers(json_body=False, provider=provider),
            ...
            if is_geeknow:
                ...
                request_data["model"] = requested_model
                try:
                    return await generate_geeknow_video(
                        client,
                        request_data,
                        base_url=base_url,
                        headers=api_headers(json_body=False, provider=provider),
            ...
            if is_megabyai:
                ...
                request_data["model"] = requested_model
                try:
                    return await generate_megabyai_video(
                        client,
                        request_data,
                        base_url=base_url,
                        headers=api_headers(json_body=False, provider=provider),
            ...
            if is_sudashui_video_generations:
                ...
                request_data["model"] = requested_model
                try:
                    return await generate_sudashui_video(
                        client,
                        request_data,
                        base_url=base_url,
                        headers=api_headers(json_body=False, provider=provider),
...
if __name__ == "__main__":
    ...
    # Windows 无控制台打包后 sys.stdout 为 None，必须显式关闭颜色探测，
    # 否则 Uvicorn 默认日志格式器调用 isatty() 时会导致后端启动失败。
