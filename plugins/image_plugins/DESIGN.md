...
- 用独立模块承载 aicost 与七牛 Modelink 图片协议，减少 `main.py` 中的供应商细节。
...
- 新增 `qiniu-image` 插件模式。
- 按 2026-08-10 的七牛 Modelink OpenAPI 接入 Fal Queue 异步生图。
- 参考图由宿主复用视频素材预检/上传链路转换为公网 URL，插件不读取本地文件。
