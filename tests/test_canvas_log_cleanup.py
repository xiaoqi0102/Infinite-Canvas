...
class CanvasLogCleanupTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cleanup_preserves_media_referenced_by_video_task(self):
        path, url = self.generated_file("video-task.mp4")
        self.video_tasks.write_text(
            json.dumps({"canvas_video_1": {"status": "succeeded", "videos": [url]}}),
            encoding="utf-8",
        )
        self.write_canvas("video_task_owner", [{"id": "log-1", "outputs": [url]}])
